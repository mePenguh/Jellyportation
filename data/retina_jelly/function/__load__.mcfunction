...
execute unless score __math__.seed retina.__variable__ matches -2147483648..2147483647 run function retina_jelly:__private__/math_random/setup
...
data merge storage retina_jelly:input {HorizontalCount:1b,VerticalCount:1b,CenteredCount:0b,SpreadFactor:[100,100],EndpointEntity:0b}
execute unless data storage retina_jelly:input MaxRecursionDepth run data merge storage retina_jelly:input {MaxRecursionDepth:50}
execute unless data storage retina_jelly:input TargetBlocks run data merge storage retina_jelly:input {TargetBlocks:true}
execute unless data storage retina_jelly:input TargetEntities run data merge storage retina_jelly:input {TargetEntities:true}
data merge storage retina_jelly:output {ContactCoordinates:[],ContactSurface:[],Distance:0.0d,HitFace:"",PlacingPosition:[],Target:"",TargetedBlock:[]}
execute store success score $old_version_installed retina.__variable__ run scoreboard players add * retina 1
execute if score $old_version_installed retina.__variable__ matches 1.. run function retina_jelly:__private__/if_else/33
return 1